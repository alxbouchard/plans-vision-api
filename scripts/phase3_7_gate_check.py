...
MCAF-COMPLIANT: This script reads payloads from the Visual Guide (stable_rules_json),
NOT from hardcoded constants. The guide must be generated via /analyze first.
...
Usage:
    # Using a guide JSON file:
    python scripts/phase3_7_gate_check.py --guide path/to/stable_rules.json

    # Using project ID (reads from database):
    python scripts/phase3_7_gate_check.py --project-id <uuid>

    # Baseline mode (no exclude payloads - for comparison only):
    python scripts/phase3_7_gate_check.py --baseline
...
from src.agents.schemas import RulePayload, RuleKind, FinalRule
...
def load_payloads_from_guide(guide_path: Path) -> list[RulePayload]:
    ...
    for rule in stable_rules:
        ...
        if payload_data:
            try:
                payload = RulePayload(**payload_data)
...
def load_payloads_from_db(project_id: UUID) -> list[RulePayload]:
    ...
    cursor.execute(
        "SELECT stable_rules_json FROM projects WHERE id = ?",
        (str(project_id),)
    )
    row = cursor.fetchone()
    conn.close()
    ...
    if not row or not row[0]:
        raise ValueError(f"No stable_rules_json found for project {project_id}")
...
def get_baseline_payloads() -> list[RulePayload]:
    """Get minimal baseline payloads (NO exclude rules).

    Used only for baseline comparison to show the problem before guide fix.
    These are the bare minimum payloads that would come from any guide.
    """
    return [
        RulePayload(
...
def validate_gate(
    ...
    # Count payload types
    exclude_payloads = len([p for p in payloads if p.kind == RuleKind.EXCLUDE])
...
async def main() -> int:
    ...
    parser.add_argument(
        "--baseline",
        action="store_true",
        help="Run with baseline payloads (no exclude) for comparison"
    )
    ...
    if args.guide:
        ...
        payloads = load_payloads_from_guide(args.guide)
    elif args.project_id:
        print(f"Loading payloads from database for project: {args.project_id}")
        payloads = load_payloads_from_db(UUID(args.project_id))
    elif args.baseline:
        print("BASELINE MODE: Using minimal payloads WITHOUT exclude rules")
        print("This shows the problem state before guide generates exclude payloads")
        payloads = get_baseline_payloads()
    else:
        print("ERROR: Must specify --guide, --project-id, or --baseline")
        print("  --guide path/to/stable_rules.json")
        print("  --project-id <uuid>")
        print("  --baseline (for comparison only)")
        return 1

    # Report payload summary
    exclude_count = len([p for p in payloads if p.kind == RuleKind.EXCLUDE])
    print(f"\nPayloads loaded: {len(payloads)} (exclude: {exclude_count})")

    if exclude_count == 0 and not args.baseline:
    ...
    for p in payloads:
        ...
        if p.kind == RuleKind.EXCLUDE:
            print(f"  - {kind_str}: pattern={p.pattern[:40]}...")
        else:
    ...
    if addenda_pdf.exists():
        rooms, breakdown = await run_extraction(addenda_pdf, 0, payloads)
    ...
    if structure_pdf.exists():
        rooms, breakdown = await run_extraction(structure_pdf, 4, payloads)
        result = validate_gate(
